...
INTAKE_TRIGGERS = {
    "прими", "принять", "приём", "прием", "приходуй", "оприходуй", "оприходовать",
    "приход", "создай", "создать", "добавь", "добавить", "заведи", "завести",
    "товар", "qabul", "kirim", "qo'sh", "qosh", "qo'shish", "qoshish", "yarat",
}
_AFFIRM_YES = {
...
_LLM_SYSTEM = (
    "Ты — слой понимания языка для помощника учёта розничного магазина в "
    "Узбекистане. Пользователь пишет по-русски или по-узбекски. Твоя задача — "
    "извлечь намерение и поля из сообщения и вернуть СТРОГО JSON по схеме. "
    "Ты НИЧЕГО не выполняешь и не придумываешь значения: если поля нет в "
    "сообщении — ставь null. intent='intake' если пользователь хочет "
    "принять/создать товар; иначе 'unknown'. affirmation: 'yes'/'no'/'cancel' "
    "для ответов подтверждения, иначе null. Цены и количество — числа "
    "(например «45 тысяч» → 45000)."
)
