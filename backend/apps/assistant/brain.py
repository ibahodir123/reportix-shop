...
_AFFIRM_YES = {
    ...
    "tasdiqlayman", "tasdiq", "bo'ldi", "boldi", "xo'p", "xop",
}
...
- size — размер одежды латиницей S/M/L/XL/XXL. Буквы: «эс»→S, «эм»→M, «эль» (а также «а.л.», «эл», «al»)→L, «икс эль»→XL, «икс икс эль»→XXL. Слова: «маленький»→S, «средний»→M, «большой»→L, «очень большой»→XL. Если размер не назван — null.
- Цены и количество — числа: «сорок пять тысяч» → 45000, «двадцать» → 20. Два числа-цены без пояснений: первое — purchase_price (закуп), второе — sale_price (продажа). Количество (quantity) — только если явно названо (штук/дона/ta или отдельное небольшое число); иначе quantity=null.
- Сообщение может быть просто перечислением товара и параметров без слов-команд, например «Футболка L синий 45000 75000» — это тоже intent=intake.
- Узбекский язык: qabul/kirim/qo'shish → intent intake. Цвета: ko'k→синий, qizil→красный, oq→белый, qora→чёрный, yashil→зелёный, sariq→жёлтый. Размеры: kichik→S, o'rta→M, katta→L. Числа: «ming»→тысяча (qirq besh ming→45000, yigirma→20). Единицы: dona/ta→штук. tannarx/xarid→закуп, sotuv/narx→продажа.
...
Пример (короткое перечисление, без количества):
Сообщение: Футболка L синий 45000 75000
Ответ: {"intent":"intake","affirmation":null,"slots":{"name":"Футболка","color":"синий","size":"L","purchase_price":45000,"sale_price":75000,"quantity":null}}"""
